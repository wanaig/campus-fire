...
public class ReportExportController {
    ...
    @GetMapping("/facilities.csv") public ResponseEntity<byte[]> facilities(Authentication a){AuthenticatedUser u=requireAdmin(a);List<Map<String,Object>> rows=jdbcTemplate.queryForList("SELECT f.facility_no,f.name,t.type_name,f.campus,f.building,f.floor,f.area,f.lifecycle_status,f.manufacture_date,f.commissioned_date,f.next_maintenance_at,(SELECT GROUP_CONCAT(fc.item_name ORDER BY fc.id SEPARATOR '；') FROM facility_component fc WHERE fc.facility_id=f.id) AS component_names FROM facility f JOIN facility_type t ON t.id=f.facility_type_id ORDER BY f.id");StringBuilder c=new StringBuilder("设施编号,设施名称,类型,校区,楼栋,楼层,区域,状态,生产日期,投用日期,下次保养,登记部件\r\n");for(Map<String,Object> r:rows)c.append(String.join(",",Arrays.asList("facility_no","name","type_name","campus","building","floor","area","lifecycle_status","manufacture_date","commissioned_date","next_maintenance_at","component_names").stream().map(k->escape(String.valueOf(r.getOrDefault(k,"")))).collect(java.util.stream.Collectors.toList()))).append("\r\n");auditService.record(u.getId(),"REPORT_EXPORT","FACILITY",null,Collections.singletonMap("format","csv"));return csv(c,"facilities.csv");}
