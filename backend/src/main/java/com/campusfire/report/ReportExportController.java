...
public class ReportExportController {
    ...
    public ReportExportController(JdbcTemplate jdbcTemplate,AuthService authService,AuditService auditService){this.jdbcTemplate=jdbcTemplate;this.authService=authService;this.auditService=auditService;}
    ...
    public ResponseEntity<byte[]> inspectionRecords(Authentication authentication){
        ...
        List<Map<String,Object>> rows=jdbcTemplate.queryForList("SELECT f.facility_no,f.name,f.campus,f.building,f.floor,f.area,u.display_name AS inspector,r.submitted_at,r.photo_count,r.note FROM inspection_record r JOIN inspection_task t ON t.id=r.task_id JOIN facility f ON f.id=t.facility_id JOIN app_user u ON u.id=r.user_id LEFT JOIN inspection_record_correction c ON c.record_id=r.id WHERE c.id IS NULL ORDER BY r.submitted_at DESC");
        StringBuilder csv=new StringBuilder("设施编号,设施名称,校区,楼栋,楼层,区域,巡检人员,提交时间,照片数量,备注\r\n");
        for(Map<String,Object> row:rows){List<String> values=Arrays.asList(row.get("facility_no"),row.get("name"),row.get("campus"),row.get("building"),row.get("floor"),row.get("area"),row.get("inspector"),row.get("submitted_at"),row.get("photo_count"),row.get("note")).stream().map(v->escape(v==null?"":String.valueOf(v))).collect(java.util.stream.Collectors.toList());csv.append(String.join(",",values)).append("\r\n");}
