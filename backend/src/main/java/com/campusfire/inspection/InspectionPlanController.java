...
public class InspectionPlanController {
    ...
    @PostMapping("/{id}/generate-tasks") public ApiResponse<?> generate(@PathVariable long id,@RequestParam @DateTimeFormat(iso=DateTimeFormat.ISO.DATE) LocalDate dueDate,Authentication a){AuthenticatedUser u=requireAdmin(a);Map<String,Object> p=jdbcTemplate.queryForMap("SELECT facility_type_id,campus,assigned_user_id FROM inspection_plan WHERE id=? AND enabled=1",id);String sql="INSERT IGNORE INTO inspection_task(plan_id,facility_id,assigned_user_id,due_date) SELECT ?,f.id,?,? FROM facility f WHERE f.lifecycle_status='IN_USE' AND (? IS NULL OR f.facility_type_id=?) AND (? IS NULL OR f.campus=?)";int count=jdbcTemplate.update(sql,id,p.get("assigned_user_id"),dueDate,p.get("facility_type_id"),p.get("facility_type_id"),p.get("campus"),p.get("campus"));auditService.record(u.getId(),"INSPECTION_TASK_GENERATE","INSPECTION_PLAN",String.valueOf(id),Collections.singletonMap("count",count));return ApiResponse.success(Collections.singletonMap("createdCount",count));}
    ...
    @DeleteMapping("/{id}") public ApiResponse<?> delete(@PathVariable long id,Authentication a){AuthenticatedUser u=requireAdmin(a);Integer tasks=jdbcTemplate.queryForObject("SELECT COUNT(*) FROM inspection_task WHERE plan_id=?",Integer.class,id);if(tasks!=null&&tasks>0)throw new IllegalArgumentException("该计划已生成巡检任务，不能删除，可先停用计划");jdbcTemplate.update("DELETE FROM inspection_plan WHERE id=?",id);auditService.record(u.getId(),"INSPECTION_PLAN_DELETE","INSPECTION_PLAN",String.valueOf(id),null);return ApiResponse.success(Collections.singletonMap("deleted",true));}
